...
FORM_CLASS, _ = uic.loadUiType(os.path.join(os.path.dirname(__file__), "watering_load_dialog.ui"))
...
class WateringLoad(QtWidgets.QDialog, FORM_CLASS):
    def __init__(self, parent=None):
        ...
        self.shp_filesMonitoring = ["watering_waterMeter.shp", "watering_sensors.shp"]
    ...
    def set_online_projects_list(self):
        ...
        for i in range(0, self.response.json()["total"]):
            self.online_projects_list.append(
                (self.response.json()["data"][i]["name"], self.response.json()["data"][i]["serverKeyId"])
            )
    ...
    def set_offline_projects_list(self, combo_box, scenarios_box, scenarios_list):
        if os.path.exists(self.projects_json_file):
            ...
            WateringUtils.error_message(
                "No projects found locally. Connect to WaterIng and load a project from server."
            )
    ...
    def set_clone_projects_combo_box(self):
        ...
        self.set_offline_projects_list(
            self.clone_projects_box, self.clone_scenarios_box, self.offline_scenarios_list_clone
        )
    ...
    def open_scenario(self):
        ...
        if existScenarioOffline:
            self.open_watering_project(justCreated)
        else:
            self.handle_failed_project_load()
            return
    ...
    def is_offline_scenario_version(self):
        ...
        scenario_in_metadata = (
            self.current_project_fk in self.projects_json_data
            and self.current_scenario_fk in self.projects_json_data[self.current_project_fk].get("scenarios", {})
        )
    ...
    def createAndOpenGroups(self, scenario_path):
        ...
        shp_backupFiles = [
            "watering_demand_nodes_backup.shp",
    ...
    def zoomToProject(self):
        ...
        if not group:
            iface.messageBar().pushMessage(
                self.tr("Error"), self.tr("Failed to zoom to WaterIng layers."), level=1, duration=5
            )
            return
    ...
    def handle_failed_project_load(self):
        WateringUtils.error_message(
            self.tr(f"Cannot open {self.current_project_name}-{self.current_scenario_name} offline.")
        )
    ...
    def set_merge_projects_combo_box(self):
        ...
        self.set_offline_projects_list(
            self.merge_projects_box_sourceA, self.merge_scenarios_box_sourceA, self.offline_scenarios_list_sourceA
        )
        self.set_offline_projects_list(
            self.merge_projects_box_sourceB, self.merge_scenarios_box_sourceB, self.offline_scenarios_list_sourceB
        )
