...
class abstractServerRESTRepository():
    ...
    def postMultipleElements(self, list_of_elementsJSON):
        ...
        with requests.Session() as session:
            with ThreadPoolExecutor(max_workers=10) as executor:
                futures = [executor.submit(self.make_post_request, session, self.UrlPost, data[0], headers, params) for data in list_of_elementsJSON]
    ...
    def putMultipleElements(self, list_of_elementsJSON):
        ...
        with requests.Session() as session:
            with ThreadPoolExecutor(max_workers=10) as executor:
                futures = [executor.submit(self.make_put_request, session, self.UrlPut, data, headers, params, data[0]['serverKeyId']) for data in list_of_elementsJSON]
