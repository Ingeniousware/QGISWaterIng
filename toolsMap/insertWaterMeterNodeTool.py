...
class InsertWaterMeterNodeTool(InsertAbstractTool):
    def __init__(self, canvas, elementRepository, actionManager, toolbarManager):
        super(InsertWaterMeterNodeTool, self).__init__(canvas, elementRepository, actionManager)  
        self.toolbarManager =  toolbarManager
        print("Init at Insert Water Meter Node")
        if (QgsProject.instance().mapLayersByName("watering_waterMeter") is not None) and len(QgsProject.instance().mapLayersByName("watering_waterMeter")) != 0:
          self.demandNodeLayer = QgsProject.instance().mapLayersByName("watering_waterMeter")[0]
