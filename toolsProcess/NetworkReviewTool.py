...
class NetworkReviewTool:
        ...
        for feature_x in pipe_features:
            ...
            for feature_y in node_features:
                ...
                if not node_geom.disjoint(pipe_geom):
                    ...
                    if distance <= 5:
                        self.node_layer.dataProvider().changeAttributeValues(
                            {feature_y.id(): {field_index: unconnected}}
                        )
    ...
    def split_lines_with_lines(self, input_layer, split_layer):
        ...
        params = {"INPUT": input_layer, "LINES": split_layer, "OUTPUT": "memory:" + layer_name}
    ...
    def update_attributes(self, layer, case):
        ...
        if case == 0:  # For pipes
            ...
            for feature in layer.getFeatures():
                new_uuid = str(uuid.uuid4())[:10]
                attr_values = {id_field_index: new_uuid}
                layer.changeAttributeValues(feature.id(), attr_values)
    ...
    def button(self):
        ...
        if project.isDirty():
            ...
            if response == QMessageBox.Yes:
                ...
                self.iface.messageBar().pushMessage(
                    "Success", "Successfully identified unconnected nodes!", level=Qgis.Success, duration=6
                )
